...
export function registerPollManageActions(
  ...
  app.action(slackActionIds.closePoll, async ({ ack, body, client, logger }) => {
    await ack();
  ...
  app.action(slackActionIds.viewPollDetails, async ({ ack, body, client, logger }) => {
    await ack();
    ...
    try {
      ...
      const snapshot = await dependencies.pollStore.findSnapshotById(action.value);

      if (snapshot === null) {
        return;
      }

      if (!dependencies.authorizationService.canViewDetailedVotes(snapshot.poll, actionBody.user.id)) {
        if (actionBody.channel !== undefined) {
          await postEphemeralMessage(client, {
            channelId: actionBody.channel.id,
  ...
  app.action(slackActionIds.homeCreatePoll, async ({ ack, body, client, logger }) => {
    await ack();
  ...
  app.action(slackActionIds.homeSetFilter, async ({ ack, body, logger }) => {
    await ack();
    ...
    try {
      ...
      await dependencies.appHomeService.publishHome({
        filter: action.value === "closed" ? "closed" : "open",
        userId: actionBody.user.id,
