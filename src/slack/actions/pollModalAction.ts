...
import {
  decodePollModalMetadata,
  encodePollModalMetadata,
} from "../metadata/pollModalMetadata.js";
...
export function registerPollModalActions(app: App) {
  app.action(
    ...
    async ({ ack, body, client, logger }) => {
      ...
      try {
        ...
        const metadata = decodePollModalMetadata(
          actionBody.view.private_metadata,
        );
        ...
        const values = extractPollFormValues(
          actionBody.view.state.values,
          metadata,
        );
  ...
  app.action(
    slackActionIds.removeOptionRow,
    async ({ ack, body, client, logger }) => {
      await ack();
      ...
      try {
        ...
        values.optionTexts = values.optionTexts.slice(
          0,
          updatedMetadata.optionCount,
        );
